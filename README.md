...
#### Initiatives
Initiatives are the projects, products, or product features which will be undertaken to increase, decrease or maintain key results. Initiatives are:
...
- **Atelic** i.e. represent an ongoing activity of business (one without an specific end in and of itself)
...
### What is OKRIA API?
OKRIA is committed to offering free tools to help support adoption of the model. You can learn more about OKRIA [here](https://okria.io/). To help companies adopt the model in software applications, We've created this open-source Strapi based OKRIA management solution.
...
### 1. Install
First we'll install the required dependancies:
...
The OKRIA API repo contains a package.json file with additional application dependancies as well as Strapi configuration files specific to the OKRIA API.

Clone the repo and install the dependancies as follows:
...
**Note:** If you intend to use OKRIA API in production (and develop it further) you may with to fork the repo instead.
...
Visit this URL in the browser, and follow the provided link to navigate to http://localhost:1337/admin in order to create an administrative account and access the interface for the first tine.

### 3. Create and Manage
Once you've logged in you will be presented with the Strapi interface. You'll find 4 content types which will allow you to create OKR, initiatives and activities. Let's walk through the complete process so you can see how the OKRIA API works:

#### 3.1 Create an initiative
The first think we will do is create a master initiative. This typically be a project, product, product feature or other company initiative. We will use this to organize objectives and key results, and plan activities:
...
2. Add an objective name in the `Objective` field
   - e.g.  `Release a more delightful on-boarding experience`
...
2. Add an metric name in the `Metric` field
   - e.g. `customer on-boarding support requests` or `customer on-boarding completion rate`
...
6. Once you have completed all required fields, save the metric.
...
2. Under `Result` click `Add new entry` in order to add a key result
...
8. Save the objective in order to save your key results

#### 3.6 Create resources
In order to complete planning our initiative activities to drive key results, we will have to engage team teams or team members (our resources).

1. Click `Resources` in the sidebar and click the `Add New Resource` button
2. Add an resource name in the `Resource` field
   - e.g. `marketing team`, `product team`, or `sales team`
3. Save your resource

#### 3.7 Create activities
Now we will return to our initiative in order to define the activities that will be required to complete the initiative.

1. Click `Initiatives` in the sidebar and navigate to edit your previously create initiative
2. Under `Activity` click `Add new entry` in order to add an activity
3. Select a previously create `Resource` from the list
   e.g. `design team`
4. Identify a `Task` that the selected resource must complete in order to complete this initiative
   - e.g. `wireframe a new on-boarding experience`
...
### 4. API
Now that you have created example objectives, key results, initiatives and activities you will be able to interact with this content via a RESTful API. This will allow you in integrate the content in front-end, or third-party applications.

#### 4.1 Manage permissions
In production it will be important to ensure proper authentication of the API endpoint to ensure security of critical data. While testing in our local environment, however, we'll use a public endpoint as a proof of concept.
...
#### 5.1 Test endpoints
We can now access endpoints for each of our content types in the browser or using an API tools (e.g. Postman) in order to consume the input content, via any front-end or third-party application.
...
2. [Introduction to OKRIA concepts]()
